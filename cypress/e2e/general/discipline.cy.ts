const disciplineTestValue = "test value";
const disciplineEditTestValue = "edit test value";

const disciplineInputs = [
  { selector: "input[name='name']", testValue: disciplineTestValue },
  {
...
describe("Employee page tests", () => {
  beforeEach(() => {
    ...
    cy.intercept("GET", "**/tblDiscipline*").as("disciplineDatas");

    cy.visit("/general/discipline");

    cy.wait("@disciplineDatas").its("response.statusCode").should("eq", 200);
  });

  describe("Create discipline modal tests", () => {
    it("Render create modal correctly", () => {
      cy.get('button:has(path[d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6z"])').click();

      cy.contains("Create Discipline").should("be.visible");
    ...
    it("Pass with filling (required) inputs", () => {
      ...
      cy.contains(disciplineTestValue).should("be.visible");
    });
  });
  ...
  describe("Edit modal tests", () => {
    it("Edit a row from table", () => {
      cy.get('[role="row"]')
        .contains('[data-field="name"]', disciplineTestValue)
        .closest('[role="row"]')
        .click();
      ...
      cy.contains("Edit Discipline").should("be.visible");

      disciplineInputs.forEach((input) => {
        cy.get(input.selector).should("be.visible").should("be.enabled");
        cy.get(input.selector).clear();
        cy.fillInput(input.selector, disciplineEditTestValue);
      });

      cy.intercept("PUT", "**/tblDiscipline/*").as("updateDiscipline");

      cy.get("button").contains("Ok").click();

      cy.wait("@updateDiscipline")
        .its("response.statusCode")
        .should("be.oneOf", [200, 204]);

      cy.wait(500);

      cy.contains(disciplineEditTestValue).should("be.visible");
    });
  });
  ...
  describe("Delete modal tests", () => {
    ...
    it("Delete a row from table", () => {
      ...
      cy.get('[role="row"]').should("not.contain", disciplineTestValue);
    });
  });
});
